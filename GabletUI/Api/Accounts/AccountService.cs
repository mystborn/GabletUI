...
namespace GabletUI.Api.Accounts
{
    public class AccountService : BaseService, IAccountService
    {
        ...
        public async Task<UserModel> Register(string username, string email, string password)
        {
            ...
            var response = await GetResource<LoginResponse, RegisterRequest>(REGISTER, request);
            if (response.Error is not null)
                throw new GabletApiException(response);
        ...
        public async Task<bool> ValidateAccount(string token, string username)
        {
            ...
            var response = await GetResource<ValidateAccountResponse, ValidateAccountRequest>(VALIDATE_ACCOUNT, request);
            if (response.Error is not null)
                throw new GabletApiException(response);
        ...
        public async Task<UserModel> Login(string username, string password)
        {
            ...
            var response = await GetResource<LoginResponse, LoginRequest>(LOGIN, request);
            if (response.Error is not null)
                throw new GabletApiException(response);
        ...
        public async Task Refresh(string token)
        {
            ...
            var response = await GetResource<LoginResponse, RefreshRequest>(REFRESH, request);
            if (response.Error is not null)
                throw new GabletApiException(response);
